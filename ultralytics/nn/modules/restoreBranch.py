...
class NAFBlockOld(nn.Module):
    """轻量级 NAF Block，用于图像复原特征提取。 去除了复杂的频域操作，保留核心的 Spatial Gating 和 Channel Attention。.
    """
    ...
    def __init__(self, c, DW_Expand=2, FFN_Expand=2, drop_out_rate=0.0):
        ...
        self.conv2 = nn.Conv2d(
            in_channels=dw_channel, out_channels=dw_channel, kernel_size=3, padding=1, groups=dw_channel, bias=True
        )
...
class RestoreBranchOld(nn.Module):
    ...
    def __init__(self, *args):
        ...
        self.input_projs = nn.ModuleList(
            [
                nn.Sequential(
                    nn.Conv2d(in_c, mid_channels, kernel_size=1, bias=False), nn.BatchNorm2d(mid_channels), nn.SiLU()
                )
        ...
        # 3. 复原主体：堆叠 NAFBlocks
        self.restore_body = nn.Sequential(*[NAFBlock(mid_channels) for _ in range(num_naf_blocks)])

        # 4. 输出头：映射回 3 通道，并使用 Sigmoid 限制在 [0, 1]
        self.output_head = nn.Sequential(nn.Conv2d(mid_channels, 3, kernel_size=3, padding=1, bias=True), nn.Sigmoid())

    def forward(self, features):
        """
        Args:
            features (list[Tensor]): 来自 Backbone 的特征图列表。 假设 features[0] 是最高分辨率 (最大 H, W)。.

        Returns:
            Tensor: 复原后的图像，形状为 [B, 3, H_target, W_target]。 H_target = features[0].H * upsample_factor
        """
        ...
        if self.upsample_factor > 1:
            output_img = F.interpolate(
                output_img, scale_factor=self.upsample_factor, mode="bilinear", align_corners=False
            )
...
class RestoreBranchHasX(nn.Module):  # hasX
    def __init__(self, a=0, b=0, c=0, d=0):  # 选择的是stage123，也就是yaml里的013
        ...
        if dec_blk_nums[0] == 1:
            self.decoderStageImage = nn.Conv2d(
                in_channels=64 // width, out_channels=3, kernel_size=3, padding=1, stride=1, groups=1, bias=True
            )
        else:
            pass
...
class RestoreBranchNoX(nn.Module):  # NoX
    def __init__(self, a=0, b=0, c=0, d=0):  # 选择的是stage123，也就是yaml里的013
        ...
        if dec_blk_nums[0] == 1:
            self.decoderStageImage = nn.Conv2d(
                in_channels=32 // width, out_channels=3, kernel_size=3, padding=1, stride=1, groups=1, bias=True
            )
        else:
            pass
