...
from .basicsr.models.archs.NAFNet_arch import Decoder
...
class DecoderOld(nn.Module):
    def __init__(self, input_nc, output_nc):
        ...
        self.conv_1 = nn.Sequential(
            nn.ConvTranspose2d(512, 256, 3, stride=2, padding=1, output_padding=1), nn.BatchNorm2d(256), nn.SiLU()
        )

        # inputsize:256*40*40(concat feat2), outputsize:128*80*80
        self.conv_2 = nn.Sequential(
            nn.ConvTranspose2d(512, 128, 3, stride=2, padding=1, output_padding=1), nn.BatchNorm2d(128), nn.SiLU()
        )

        # inputsize:128*80*80(concat feat1), outputsize:64*160*160
        self.conv_3 = nn.Sequential(
            nn.ConvTranspose2d(256, 64, 3, stride=2, padding=1, output_padding=1), nn.BatchNorm2d(64), nn.SiLU()
        )

        self.upsample = nn.Upsample(scale_factor=4, mode="bilinear", align_corners=True)

        # Output layer
        self.conv_4 = nn.Sequential(nn.ReflectionPad2d(3), nn.Conv2d(64, output_nc, 7), nn.Tanh())
...
class YOLOPAFPN(nn.Module):
    def __init__(
        ...
        self.decoder = Decoder(
            use_indices=[0, 1, 2, 3, 4], dec_blk_nums=[1, 1, 1, 1, 1]
        )  # (use_indices=[0, 1, 2, 3, 4], dec_blk_nums=[1, 1, 1, 1, 1])

    def forward(self, input):

        Batchsize = input.shape[0] // 2
        if self.training:
            input, _clear_x = input.split(
                (Batchsize, Batchsize), dim=0
            )  # split haze and clear images (Batchsize, Batchsize)
        out_features = self.backbone.forward(input)

        [feat1, feat2, feat3, feat4, feat5] = [
            out_features[f] for f in self.in_features
        ]  # feat1[3, 32, 1024, 1024] feat2[3, 64, 512, 512] feat3[3, 128, 256, 256] feat4[3, 256, 128, 128] feat5[3, 512, 64, 64]
        # -------------------------------------------#
        #   20, 20, 1024 -> 20, 20, 512s
        # -------------------------------------------#
        P5 = self.lateral_conv0(feat5)
        # -------------------------------------------#
        #  20, 20, 512 -> 40, 40, 512
        # -------------------------------------------#
        ...
        # -------------------------------------------#
        P5_upsample = torch.cat([P5_upsample, feat4], 1)
        # -------------------------------------------#
        #   40, 40, 1024 -> 40, 40, 512
        # -------------------------------------------#
        ...
        P4_upsample = torch.cat([P4_upsample, feat3], 1)
        # -------------------------------------------#
        #   80, 80, 512 -> 80, 80, 256
        # -------------------------------------------#
        ...
        dehazing = self.decoder([feat1, feat2, feat3, feat4, feat5])
