...
class YOLOPAFPN(nn.Module):
    def __init__(self, depth = 1.0, width = 1.0, in_features = ("stem","dark2","dark3", "dark4", "dark5"), in_channels = [256, 512, 1024], depthwise = False, act = "silu"):
        ...
        self.decoder = Decoder(use_indices=[0, 1, 2, 3,4], dec_blk_nums=[1, 1, 1, 1,1])#(use_indices=[0, 1, 2, 3, 4], dec_blk_nums=[1, 1, 1, 1, 1])

    def forward(self, input):
        
        Batchsize = input.shape[0]//2
        if self.training:
            input, clear_x = input.split((Batchsize, Batchsize), dim=0)  # split haze and clear images (Batchsize, Batchsize)
        out_features            = self.backbone.forward(input)

        [feat1, feat2, feat3,feat4,feat5]   = [out_features[f] for f in self.in_features]#feat1[3, 32, 1024, 1024] feat2[3, 64, 512, 512] feat3[3, 128, 256, 256] feat4[3, 256, 128, 128] feat5[3, 512, 64, 64] 
        #-------------------------------------------#
