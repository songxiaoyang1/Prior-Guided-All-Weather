...
class Decoder(nn.Module):
    def __init__(self, width=32, use_indices=[0, 1, 2, 3, 4], dec_blk_nums=[1, 1, 1, 1, 1], 
                 dw_expand=2, ffn_expand=2, out_channels=3, target_size=2048):
        """
        Args:
            width (int): 基础通道数 (Default 32).
            use_indices (list[int]): 选用的编码器层级索引 (0-4)。
                                     必须从小到大排序。
        ...
        for i in range(self.num_levels):
            ...
            decoder_layer = nn.Sequential(
                *[NAFBlock(current_chan, DW_Expand=dw_expand, FFN_Expand=ffn_expand) for _ in range(dec_blk_nums[rev_idx])]
            )
            ...
            if rev_idx > 0:
                ...
                up_module = nn.Sequential(
                    nn.Conv2d(current_chan, next_chan * 4, kernel_size=1, bias=False),
                    nn.PixelShuffle(2)
                )
        ...
        # 3. 最终输出头
        # 将通道映射到 out_channels
        self.final_conv = nn.Conv2d(self.selected_channels[0], out_channels, kernel_size=3, padding=1, stride=1, bias=True)
