...
from basicsr.utils import FileClient, imfrombytes, img2tensor, imwrite, set_random_seed, tensor2img
...
def parse_options(is_train=True):
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The path to the input left image. For stereo image inference only.",
    )
    parser.add_argument(
        "--input_r_path",
        type=str,
        required=True,
        help="The path to the input right image. For stereo image inference only.",
    )
    parser.add_argument(
        "--output_l_path",
        type=str,
        required=True,
        help="The path to the output left image. For stereo image inference only.",
    )
    parser.add_argument(
        "--output_r_path",
        type=str,
        required=True,
        help="The path to the output right image. For stereo image inference only.",
    )
...
def imread(img_path):
    ...
    try:
        ...
        raise Exception(f"path {img_path} not working")
