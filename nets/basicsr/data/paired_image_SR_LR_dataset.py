...
class PairedImageSRLRDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        gt_path_L = os.path.join(self.gt_folder, f"{index + 1:04}_L.png")
        gt_path_R = os.path.join(self.gt_folder, f"{index + 1:04}_R.png")
        ...
        try:
            ...
            raise Exception(f"gt path {gt_path_L} not working")

        img_bytes = self.file_client.get(gt_path_R, "gt")
        try:
            img_gt_R = imfrombytes(img_bytes, float32=True)
        except:
            raise Exception(f"gt path {gt_path_R} not working")

        lq_path_L = os.path.join(self.lq_folder, f"{index + 1:04}_L.png")
        lq_path_R = os.path.join(self.lq_folder, f"{index + 1:04}_R.png")
        ...
        try:
            ...
            raise Exception(f"lq path {lq_path_L} not working")

        img_bytes = self.file_client.get(lq_path_R, "lq")
        try:
            img_lq_R = imfrombytes(img_bytes, float32=True)
        except:
            raise Exception(f"lq path {lq_path_R} not working")
        ...
        if self.opt["phase"] == "train":
            ...
            # random crop
            img_gt, img_lq = paired_random_crop(img_gt, img_lq, gt_size, scale, gt_path_L)
            # flip, rotation
...
class PairedStereoImageDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt["phase"] == "train":
            ...
            # flip, rotation
            imgs, _status = augment(
                [img_gt, img_lq],
                self.opt["use_hflip"],
                self.opt["use_rot"],
                vflip=self.opt["use_vflip"],
                return_status=True,
            )
