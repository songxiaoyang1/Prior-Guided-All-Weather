...
class PairedImageSRLRFullImageMemoryDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_lq = self.lqs[index].copy().astype(np.float32) / 255.0
        #
        # print('index .. ', index)
        ...
        #
        #
        img_gt = self.gts[index].copy().astype(np.float32) / 255.0
        ...
        if self.opt["phase"] == "train":
            ...
            if "flip_LR" in self.opt and self.opt["flip_LR"] and np.random.rand() < 0.5:
            ...
            if self.opt.get("naive_inverse_RGB"):
                # for i in range(3):
                if np.random.rand() < 0.5:
                    img_gt = 1 - img_gt
                    img_lq = 1 - img_lq
